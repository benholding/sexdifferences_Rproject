...
women_attention_rt_prior <- c(set_prior("normal(0,1)", class = "Intercept"),
             ...
             set_prior("normal(0,1)", class = "b", coef = "menstrual_conditionContraceptive"),
             set_prior("normal(0,1)", class = "b", coef = "sleep_loss:menstrual_conditionContraceptive"),
...
women_stm_slesi <- slesi_data$stm %>% 
  mutate(sleep_loss = sd,
         ...
         id = as.factor(id),
         menstrual_condition = factor(menstrual_condition, levels = c("Follicular", "Luteal", "Contraceptive"))) %>% 
  select(id, female,sleep_loss, mistake,menstrual_condition) %>% 
  drop_na(menstrual_condition) %>% 
  filter(female==1)
...
women_maths_slesi <- slesi_data$math  %>% 
  mutate(sleep_loss = sd,
         ...
         menstrual_condition = factor(menstrual_condition, levels = c("Follicular", "Luteal", "Contraceptive"))) %>% 
  select(id, female,sleep_loss, mistake,menstrual_condition,response_time) %>% 
  drop_na(menstrual_condition) %>% 
  filter(female==1)
...
women_priors_maths_rt <- c(set_prior("normal(5000,500)", class = "Intercept"),
                     ...
                     set_prior("normal(0,500)", class = "b", coef = "menstrual_conditionContraceptive"),
                     set_prior("normal(0,500)", class = "b", coef = "sleep_loss:menstrual_conditionContraceptive"),
...
women_stroop_data <- slesi_data$stroop %>% 
  mutate(sleep_loss = sd,
         ...
         menstrual_condition = factor(menstrual_condition, levels = c("Follicular", "Luteal", "Contraceptive"))) %>% 
  select(id, female,sleep_loss, mistake, reaction_time,incongruent,menstrual_condition) %>% 
  filter(female==1)
...
women_priors_stroop_rt <- c(set_prior("normal(500,50)", class = "Intercept"),
                            ...
                            set_prior("normal(0,50)", class = "b", coef = "menstrual_conditionContraceptive"),
                            set_prior("normal(0,50)", class = "b", coef = "sleep_loss:menstrual_conditionContraceptive"),
...
women_slesi_rating_data <- slesi_data$ratings %>% 
  mutate(sleep_loss = sd,
         ...
         id = as.factor(id),
         menstrual_condition = factor(menstrual_condition, levels = c("Follicular", "Luteal", "Contraceptive"))) %>% 
  select(id, female,sleep_loss, test_type, sleepiness = rating1, performance = rating2, effort = rating3,menstrual_condition)%>% 
  drop_na(menstrual_condition) %>% 
  filter(female==1)
...
women_sleepybrain_rating_data <- sleepybrain_data$ratings %>% 
  mutate(sleep_loss = if_else(sleep_restricted == 1, 0.375, 0),
         test_type = "W",
         menstrual_condition = factor(menstrual_condition, levels = c("Follicular", "Luteal", "Contraceptive")))  %>% 
  select(id, female,sleep_loss,sleepiness=working_mem_KSS_rating,performance = working_mem_perform_rating, effort = working_mem_effort_rating, test_type, menstrual_condition)%>% 
  drop_na(menstrual_condition) %>% 
  filter(female==1)
